...
full stack while expressing a quarter of it. Results are multi-seed directions with per-seed numbers
throughout; we release the instrument and raw per-fact timelines.
\end{abstract}
...
hardware with 3--5 seeds per condition. The stream is synthetic and collision-free; real facts
correlate, conflict, and get revised --- revision streams (e.g., CounterFact-style) are future work.
Recall probes are form-matched to the writes (this inflates all absolute recall numbers; it is held
constant across comparisons); paraphrase probing is future work. The self-test that gates replay is
...
substrates even where the magnitudes do not. All of this is invisible to aggregate accuracy; the

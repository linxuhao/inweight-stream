...
adapter capacity, the last third of the stack alone matches full-stack replay capacity while the first
third retains almost nothing --- opposite to the early-layer/FFN advantage reported for batch injection
...
from the same instrument prior to the recognition meter (3-seed finals shown; the released fine-probe
pair gives 3/6, recovery 10\%); its rank/timescale sweep is flat (2--5/48) across
$dt\in\{0.1,0.5\}$, cascade depth $\{8,32\}$. Firewall: $+0.00$ in every run.}
\label{tab:main}
\end{table}
...
latent} --- is, to our knowledge, new, and it suggests a candidate design rule that our results support:
\textbf{pair a suppression-inducing protector with an error-gated re-instatement schedule.} In cognitive terms the
...
replay  & 3.0 \hfill (30\%) & 12.3 \hfill (69\%) & \textbf{21.3} \hfill (74\%) & 19.0 \hfill (76\%) \\
...
alone matches full-stack replay capacity at a third of the parameters (21.3 vs.\ 19.0, and with
the tightest seed triple in the study, 21/22/21; an earlier superseded pair of full-stack runs of
the same arm gave 29/18 --- seed variance dominates this comparison), and it is the only
...
\bibitem[Lu et al.(2026)]{mssr2026} Lu, Y., He, Y., Chen, J., \& Zha, H. MSSR: Memory-aware
adaptive replay for continual LLM fine-tuning. arXiv:2603.09892.
...
Driver: \texttt{accum.py} (mechanism, \texttt{--layers}, \texttt{--replay-policy}, recognition
margins logged at every probe); \texttt{analyze.py} re-derives all tables from the shipped raw
per-fact timelines (\texttt{results/accum\_*.json}) with the pinned recovery definition;
\texttt{reproduce.sh} is the full run matrix. Hyperparameters as in \S\ref{sec:instrument}; seeds
1234/2025/777 throughout.
